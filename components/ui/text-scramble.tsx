        ...
        {displayText.split("").map((char, i) => (
          <span
            ...
            className={`inline-block transition-all duration-150 ${
              isScrambling && char !== text[i] ? scrambleClassName : "text-foreground"
            }`}
            style={{
              transitionDelay: `${i * 10}ms`,
              minWidth: text[i] === " " ? "0.5em" : undefined,
            }}
          >
            {/* Use non-breaking space so inline-block doesn't collapse the gap */}
            {char === " " ? " " : char}
          </span>
        ))}
      </span>
